...
export default {
	name: "ElectrolyteCoatingPage",
	mixins: [coatingMixins],
	methods: {
		ConfirmEditEventHandler(){
			this.editCoatingFormConfirmHandler(COATING_TYPES.ELECTROLYTE.key);
		},

		getPageData(page){
			this.listCoatings({type: COATING_TYPES.ELECTROLYTE.key, page});
		},
	},
	mounted(){
		this.listCoatings({type: COATING_TYPES.ELECTROLYTE.key, page: 1});
