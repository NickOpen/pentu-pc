...
export default {
	name: "VacuumCoatingPage",
	mixins: [coatingMixins],
	methods: {
		ConfirmEditEventHandler(){
			this.editCoatingFormConfirmHandler(COATING_TYPES.VACUUM.key);
		},

		getPageData(page){
			this.listCoatings({type: COATING_TYPES.VACUUM.key, page});
		},
	},
	mounted(){
		this.listCoatings({type: COATING_TYPES.VACUUM.key, page: 1});
