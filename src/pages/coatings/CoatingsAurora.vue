...
export default {
	name: "AuroraCoatingPage",
	mixins: [coatingMixins],
	methods: {
		ConfirmEditEventHandler(){
			this.editCoatingFormConfirmHandler(COATING_TYPES.AURORA.key);
		},

		getPageData(page){
			this.listCoatings({type: COATING_TYPES.AURORA.key, page});
		},
	},
	mounted(){
		this.listCoatings({type: COATING_TYPES.AURORA.key, page: 1});
